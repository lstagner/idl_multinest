...
FUNCTION cluster_data,data,ellipsoid=ellipsoid

	FORWARD_FUNCTION cluster_data
	data_info=size(data)
	ncol=data_info[2] ;;NUMBER OF DATA POINTS
	nrow=data_info[1] ;;NUMBER OF DIMENSIONS
	...
	if not keyword_set(ellipsoid) then begin
		...
		evals1=eigenql(cov1,/double,eigenvectors=evecs1)
	...
	if nwh1 lt nrow+1 or nwh2 lt nrow+1 then begin
		...
		evals21=eigenql(cov21,/double,eigenvectors=evecs21)
		evals22=eigenql(cov22,/double,eigenvectors=evecs22)
		sevecs21=evecs21
		sevecs22=evecs22
       ...
       for i=0,n_elements(evals22)-1 do begin
        ...
        BIC2=(ncol1+ncol2)*nrow1*alog(2.0*3.14159265)+ncol1*alog(detcov21)+ncol2*alog(detcov22)+a21+a22 $
